...
$grid->salt("dldli3ksa");
//$grid->set_fields("unix_timestamp(calldate) as calldate,concat(IF(dst='".$extension."' OR dstchannel LIKE 'SIP/$extension-________' OR dstchannel LIKE 'PJSIP/$extension-________',src,dst),'!',clid) as number,duration,disposition,uniqueid,recordingfile");
$grid->set_fields("unix_timestamp(calldate) as calldate,concat(IF(dst='".$extension."' OR dstchannel LIKE 'SIP/$extension-________' OR dstchannel LIKE 'PJSIP/$extension-________',src,dst),'!',clid) as number,duration,disposition,uniqueid");
...
//$grid->add_display_filter('recordingfile','downloadfile');
...
function downloadfile($filename) {
   ...
   $partes = preg_split("/-/",$filename);
   $year = substr($partes[3],0,4);
   $month = substr($partes[3],4,2);
   $day = substr($partes[3],6,2);
   $filename = "/var/spool/asterisk/monitor/$year/$month/$day/$filename";
