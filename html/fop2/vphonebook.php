...
if(isset($_REQUEST['id'])) {
    // We query the image file on phonebook if any
    $res=$db->consulta("SELECT id FROM visual_phonebook WHERE phone1='%s' OR phone2='%s' LIMIT 1",$_REQUEST['id'],$_REQUEST['id']);
    if($db->num_rows($res)>0) {
        $row=$db->fetch_assoc($res);
        echo $row['id'];
    }
...
function query_phonebook($texto) {
    ...
    if($db->num_rows($res)>0) {
        while($row=$db->fetch_assoc($res)) {
            ...
            if($row['phone1']<>"") {
                $results[] = array('name'=>'','value'=>"$htmlname <$phone1>");
            }
            if($row['phone2']<>"") {
                $results[] = array('name'=>'','value'=>"$htmlname <$phone2>");
            }
        }
    } 
      return $results;
