...
function insertAuditoriaToDB($data){
    ...
    if ($pDB->query($sql, array($data['fecha'], $data['user'], $data['description'], $data['module']))) {
        $bExito = TRUE;
    } else {
        $pDB->errMsg = $pDB->_DB->errMsg;
    }  */  
...
function writeLOG($logFILE, $log)
{
    ...
    $getTypeAndUser = explode(" ",$getStringTypeAndUser[0]);
    $type = $getTypeAndUser[0];
    $user = $getTypeAndUser[1];
    $dateNow = date("Y-m-d H:i:s");
    $getStringModule = explode('"',$log);
    $module = count($getStringModule) > 1 ? $getStringModule[1] : '';
    $description = $getStringTypeAndUser[1];
