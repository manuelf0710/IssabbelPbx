    ...
    <div class="neo-table-header-row">
        {foreach from=$arrActions key=k item=accion name=actions}
            {if $accion.type eq 'link'}
                <a href="{$accion.task}" class="x-neo-table-action" {if !empty($accion.onclick)} onclick="{$accion.onclick}"
                    {/if}>
                    <div class="neo-table-header-row-filter">
                        <button type="button" name="{$accion.task}" value="{$accion.alt}" class="neo-table-toolbar-button"
                            {if !empty($accion.ocolor)}
                            style="background-color:#{$accion.ocolor}; border:1px solid #{$accion.ocolor};" {/if}>
                            {if !empty($accion.iconclass)}<i class="{$accion.iconclass}"></i> 
                            {elseif !empty($accion.icon)}<img
                                border="0" src="{$accion.icon}" align="absmiddle" />{/if}{$accion.alt}
                        </button>
                    </div>
                </a>
            {elseif $accion.type eq 'button'}
                <div class="neo-table-header-row-filter">
                    <button type="button" name="{$accion.task}" value="{$accion.alt}" {if !empty($accion.onclick)}
                            onclick="{$accion.onclick}" {/if} class="neo-table-toolbar-button" {if !empty($accion.ocolor)}
                        style="background-color:#{$accion.ocolor}; border:1px solid #{$accion.ocolor};" {/if}>
                        {if !empty($accion.iconclass)}<i class="{$accion.iconclass}"></i> 
                        {elseif !empty($accion.icon)}<img
                            border="0" src="{$accion.icon}" align="absmiddle" />{/if}{$accion.alt}
            ...
            {elseif $accion.type eq 'submit'}
                <div class="neo-table-header-row-filter">
                    <button type="submit" name="{$accion.task}" value="{$accion.alt}" {if !empty($accion.onclick)}
                            onclick="{$accion.onclick}" {/if} class="neo-table-toolbar-button" {if !empty($accion.ocolor)}
                        style="background-color:#{$accion.ocolor}; border:1px solid #{$accion.ocolor};" {/if}>
                        {if !empty($accion.iconclass)}<i class="{$accion.iconclass}"></i> 
                        {elseif !empty($accion.icon)}<img
            ...
            {elseif $accion.type eq 'text'}
                <div class="neo-table-header-row-filter" style="cursor:default">
                    <input type="text" id="{$accion.name}" name="{$accion.name}" value="{$accion.value}"
                        {if !empty($accion.onkeypress)} onkeypress="{$accion.onkeypress}" {/if} style="height:22px" />
                    <input type="submit" name="{$accion.task}" value="{$accion.alt}" class="neo-table-action" />
                </div>
            {elseif $accion.type eq 'combo'}
                <div class="neo-table-header-row-filter" style="cursor:default">
                    <select name="{$accion.name}" id="{$accion.name}" {if !empty($accion.onchange)}
                        onchange="{$accion.onchange}" {/if}>
            ...
            {if $pagingShow and $numPage>1}
                {if $start<=1}
                    <i class="fa fa-step-backward" style="color:#ccc;"></i>&nbsp;<i class="fa fa-backward"
                        style="color:#ccc"></i>
                {else}
                    <a href="{$url}&nav=start&start={$start}" class="fa fa-step-backward neo-navigation-arrow-active"
                        alt='{$lblStart}'></a>
                    <a href="{$url}&nav=previous&start={$start}" class="fa fa-backward neo-navigation-arrow-active"
                        alt='{$lblPrevious}'></a>
                {/if}
                &nbsp;{$lblPage}&nbsp;
                <input type="text" value="{$currentPage}" size="2" align="absmiddle" name="page"
                    id="pageup" />&nbsp;{$lblof}&nbsp;{$numPage}
                ...
                {else}
                    <a href="{$url}&nav=next&start={$start}" class="fa fa-forward neo-navigation-arrow-active"
                        alt='{$lblNext}'></a>
                    <a href="{$url}&nav=end&start={$start}" class="fa fa-step-forward neo-navigation-arrow-active"
                        alt='{$lblEnd}'></a>
                {/if}
            {/if}
        </div>
            ...
            {foreach from=$arrFiltersControl key=k item=filterc name=filtersctrl}
                <span class="neo-filter-control"><i>{$filterc.msg}</i>&nbsp;
                    {if $filterc.defaultFilter eq no}
                        <a href="{$url}&name_delete_filters={$filterc.filters}" style="color:#ccc;text-decoration:none;"><i
